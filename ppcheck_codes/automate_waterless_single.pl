open(OFH,">>final_results.xls");

$pdb = shift;
$chain1 = shift;
$chain2 = shift;

@data=($pdb, $chain1, $chain2);
...
system("./ncbs -h atoms.pdb h-fixed.pdb -cid $data[1] $data[2] >hbond.OFH");
system("./ncbs -d atoms.pdb h-fixed.pdb -cid $data[1] $data[2] -cutoff 10.0 >ddd.OFH");
system("./pro-procheck 8 $data[1] $data[2] h-fixed.pdb");
print OFH ("$data[0] \($data[1]-$data[2]\) \t");
