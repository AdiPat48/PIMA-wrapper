...
# To keep residue and chain information from hbond.dat file in a proper format in a new file "hbondd.dat"
...
if(($temp17[0] eq "") && ($temp17[7]<10) && ($temp17[9]<0))
{
print Write17 "$line17";
}
elsif(($temp17[0] eq "") && ($temp17[7]<10) && ($temp17[9]>0))
{
...
printf Write6 "%11s",${temp6[6]};
printf Write6 "%10s",${temp6[8]};
printf Write6 "\n";
}
close Write6;
...
printf Write7 "%11s",${temp7[6]};
printf Write7 "%10s",${temp7[8]};
printf Write7 "\n";
}
close Write7;
...
$line8 =~ s/\n//;
@temp8=split(/\s+/,$line8);
@ansh8="$temp8[2]$temp8[1]$temp8[3] "."$temp8[7]$temp8[6]$temp8[8] ";
...
{
@temp9=split(/\s+/,$line9);
  $key9="$temp9[2]$temp9[1]$temp9[3] ";
  $value9="$temp9[7]$temp9[6]$temp9[8] ";
  @ansh9="$key9".' '."$value9".' ';
...
@temp10=split(/\s+/,$line10);
  $key10="$temp10[2]$temp10[1]$temp10[3] ";
  $value10="$temp10[7]$temp10[6]$temp10[8] ";
...
@temp11=split(/\s+/,$line11);
  $key11="$temp11[2]$temp11[1]$temp11[3] ";
  $value11="$temp11[7]$temp11[6]$temp11[8] ";
...
print Write13 "Hydrogen Bond Energy : $energy13 kJ/mol \n";
print output "$energy13\t";
...
if($energy14 == 0)
{
 print Write13 "Electrostatic Energy : 0 kJ/mol\n";
 print output "0\t";
}
else
{
 print Write13 "Electrostatic Energy : $energy14 kJ/mol\n";
 print output "$energy14\t";
}
...
if($temp16[0] eq 'van')
 {
 $energy15=$temp16[5];
 print Write13 "$part15\n";
 print output "$energy15\t";
 }
elsif($temp16[0] eq 'Total')
 {
 $energy_total=($energy13+$energy14+$energy15);
 print Write13 "Total Stabilizing Energy : $energy_total kJ/mol\n";
 print output "$energy_total\t";
 }
else
 {
 }
...
print Write13 "Number of interaface residues are: $number16\n";
print output "$number16\t";
print Write13 "Normalized Energy per residue is: $norm_energy16 kJ/mol\n";
print output "$norm_energy16 \n";
}
...
{
$anshs3=substr $tempo2,-1;
$anshs4=substr $tempo2,3,-1;
 if($chain1 eq "")
 {
 print Writ1 "$tempo2 ";
 print Writ3 "$anshs4 ";
