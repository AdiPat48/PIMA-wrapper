...
open(Read1,"atoms.pdb");
@data1=<Read1>;
close(Read1);

open(Read2,"atoms.pdb");
@data2=<Read2>;
close(Read2);
...
{
$distance=sqrt((($temp2[6]-$temp1[6])*($temp2[6]-$temp1[6]))+(($temp2[7]-$temp1[7])*($temp2[7]-$temp1[7]))+(($temp2[8]-$temp1[8])*($temp2[8]-$temp1[8])));
if(($distance>0) && ($distance<=6.0))
{
print Write1 "Distance between $temp2[3]$temp2[5]$temp2[4] & $temp1[3]$temp1[5]$temp1[4] is $distance\n";
}
}
}
}
...
print Write4 "$chain1$num1$res1	$chain2$num2$res2\n";
}
...
foreach $line5 (@data5)
{
  ...
  print Write8 "$temp5[0]	I	$temp5[1]\n";
  }
...
system ('cat network.txt network2.txt > final_networks.txt');
