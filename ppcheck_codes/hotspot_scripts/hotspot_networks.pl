...
{
@temp1=split(/\s+/,$line1);
printf Write1 "${temp1[3]}${temp1[1]}${temp1[2]}	${temp1[8]}${temp1[6]}${temp1[7]}\n";
printf Write1 "${temp1[8]}${temp1[6]}${temp1[7]}	${temp1[3]}${temp1[1]}${temp1[2]}\n";
...
printf Write1 "${temp12[3]}${temp12[1]}${temp12[2]}	${temp12[8]}${temp12[6]}${temp12[7]}\n";
printf Write1 "${temp12[8]}${temp12[6]}${temp12[7]}	${temp12[3]}${temp12[1]}${temp12[2]}\n";
...
printf Write1 "${temp13[3]}${temp13[1]}${temp13[2]}	${temp13[8]}${temp13[6]}${temp13[7]}\n";
printf Write1 "${temp13[8]}${temp13[6]}${temp13[7]}	${temp13[3]}${temp13[1]}${temp13[2]}\n";
...
printf Write1 "${temp14[3]}${temp14[1]}${temp14[2]}	${temp14[8]}${temp14[6]}${temp14[7]}\n";
printf Write1 "${temp14[8]}${temp14[6]}${temp14[7]}	${temp14[3]}${temp14[1]}${temp14[2]}\n";
...
system ('uniq sorted.txt > networks.txt');
...
$line2=~s/\n//g;
@temp2=split(/\t+/,$line2);
 $chain1=substr($temp2[0],0,1);
 $res1=substr($temp2[0],-3);
 $num1=substr($temp2[0],1,-3);
 $chain2=substr($temp2[1],0,1);
 $res2=substr($temp2[1],-3);
 $num2=substr($temp2[1],1,-3);
 if($count eq 0)
 {
$count++;
$resi1=$temp2[0];
...
print Write3 "$chain1$num1$res1	$chain2$num2$res2\n";
 ...
 if(($temp2[0] eq $resi1) && ($temp2[1] ne $resi2))
{
$count++;
$resi1=$temp2[0];
$resi2=$temp2[1];
...
open(W3,">degrees_residues.txt");
open(R3,"information.txt");
@data3=<R3>;
close(R3);
...
foreach $line6 (@data6)
{
  ...
  $line6=~s/\t/ /g;
  @temp6 = split(/\s+/,$line6);
  if(($temp6[0] eq $previous) && ($preva ne ""))
  {
  push(@ansh,$temp6[1]);
  $previous=$temp6[0];
  $preva=1;
...
open(Write9,">temp_avg_degree.txt");
open(Read8,"degrees_residues.txt");
@data8=<Read8>;
close(Read8);
...
open (W5,">high_degrees_residues.txt");

open (Read5,"degrees_residues.txt");
@data5=<Read5>;
close(Read5);

open (Read6, "temp_avg_degree.txt");
@data6=<Read6>;
close(Read6);
...
if($temp5[2] >= $line6)
{
printf W5 "$temp5[0]\t$temp5[2]\n";
}
}
}
close (W5);
system ('rm -rf temp_avg_degree.txt');
